...
bool Encryption::LoadCrypto(std::string name)
{
	if(!Load(name.c_str()))
	{
		printf("Failed to load %s from the operating system.\n", name.c_str());
		return false;
	}
	else
	{
		...
		{
			printf("Failed to attach Encrypt.\n");
			Unload();
			return false;
		}
		...
		{
			printf("Failed to attach DecryptUsernamePassword.\n");
			Unload();
			return false;
		}
		...
		if(delete_func == NULL)
		{
			printf("Failed to attach _HeapDeleteCharBuffer.\n");
			Unload();
			return false;
		}
	}
